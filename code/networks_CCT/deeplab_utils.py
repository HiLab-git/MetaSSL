...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #x=self.sigmoid(out)
        #print("out",out.shape)
        #x = F.interpolate(out, size=x.shape[2:], mode='bilinear', align_corners=False)
        x1=x*self.sigmoid(out)
        #print("x",x.shape)
        return x1
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=3):
        ...
        self.conv11 = nn.Conv2d(2, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        #x=y
        #x=self.sigmoid(y)
        #y = F.interpolate(y, size=x.size(1), mode='bilinear', align_corners=False)
        
        
        return x*self.sigmoid(y1)

class StatisticalInfoConnection(nn.Module):
    def __init__(self, in_channels, out_channels, original_shape):
        super(StatisticalInfoConnection, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
    ...
    def forward(self, x):
        ...
        out = self.conv1(out)
        out = self.conv2(out)
        out = F.interpolate(out, size=self.original_shape[2:], mode='bilinear', align_corners=False)
        return out
...
class CBAM(nn.Module):
    def __init__(self, in_planes, ratio=16, kernel_size=7):
        ...
        #self.sta=StatisticalInfoConnection(in_planes,in_planes)
...
class ConvB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1):
        ...
        self.relu3 = nn.ReLU()
        #self.initialize([self.conv1, self.bn1, self.conv2, self.bn2, self.conv3, self.bn3])
