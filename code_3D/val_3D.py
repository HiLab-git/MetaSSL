...
def test_all_case(net, base_dir, test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24):
    ...
    for image_path in tqdm(image_list):
        ...
        print("label0",(label==0).sum())
        print("label1",(label==1).sum())
        print("label2",(label==2).sum())
        print("label3",(label==3).sum())
        print("image.shape",image.shape)
        for i in range(1, num_classes):
            print("i:",i)
            total_metric[i-1, :] += cal_metric(label == i, prediction == i)
            print("label==i",(label==i).sum())
            print("prediction==i",(prediction==i).sum())
            print(cal_metric(label == i, prediction == i))
