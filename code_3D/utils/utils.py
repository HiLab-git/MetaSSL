...
def vote_threshold_label_selection_class_3D(pred1, pred2, cla1, cla2):
    """ 
        input:
            pred1 & pred2: logits with per-class prediction probability: B, C, H, W
            threshold: confident predictions
    ...
    cla1_0=cla1_0.unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4)
    cla1_1=cla1_1.unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4)
  
    cla2_0=cla2_0.unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4)
    cla2_1=cla2_1.unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4)
   

    if cla1_0[0]>0.9:
        #把cla1_0tensor的值限制在0.9
        cla1_0=torch.full_like(cla1_0, 0.9)
    if cla1_1[0]>0.9:
        cla1_1=torch.full_like(cla1_1, 0.9)
    if cla2_0[0]>0.9:
        cla2_0=torch.full_like(cla2_0, 0.9)
    if cla2_1[0]>0.9:
        cla2_1=torch.full_like(cla2_1, 0.9)
    ...
    pixel_thresholds1 = 1 * np.ones((2,112, 112, 80))
    pixel_thresholds2 = 1 * np.ones((2,112, 112, 80))
    pixel_thresholds1 = torch.from_numpy(pixel_thresholds1).cuda() 
    pixel_thresholds2 = torch.from_numpy(pixel_thresholds2).cuda()
